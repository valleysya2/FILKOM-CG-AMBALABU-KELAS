...
void Demo::Render() {
	...
	glUniformMatrix4fv(glGetUniformLocation(this->lightingShader, "projection"), 1, GL_FALSE, glm::value_ptr(projection));
	...
	glUniformMatrix4fv(glGetUniformLocation(this->lightingShader, "view"), 1, GL_FALSE, glm::value_ptr(view));
	
	// Setting Light Attributes
	glUniformMatrix4fv(glGetUniformLocation(this->lightingShader, "lightSpaceMatrix"), 1, GL_FALSE, glm::value_ptr(lightSpaceMatrix));
	glUniform3f(glGetUniformLocation(this->lightingShader, "viewPos"), cameraPos.x, cameraPos.y, cameraPos.z);
	glUniform3f(glGetUniformLocation(this->lightingShader, "lightPos"), -2.0f, 4.0f, -1.0f);

	// Configure Shaders
	glUniform1i(glGetUniformLocation(this->lightingShader, "diffuseTexture"), 0);
	glUniform1i(glGetUniformLocation(this->lightingShader, "shadowMap"), 1);
	...
	DrawTexturedPlane(this->lightingShader);
	...
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, depthMap);
	DrawTexturedCube(this->lightingShader);
...
void Demo::BuildShaders()
{
	...
	// ------------------------------------
	lightingShader = BuildShader("shadowMapping.vert", "shadowMapping.frag", nullptr);
